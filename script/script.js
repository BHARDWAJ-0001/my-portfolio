...
const count_projects = () => {
  fetch(url)
    ...
    .then((data) => {
      ...
      options[1].textContent = `ML Projects (${landing_projects}+)`;
      options[2].textContent = `EDA Projects (${api_projects}+)`;
      options[3].textContent = `Dashboards (${personal_projects}+)`;
      options[4].textContent = `Capstone Projects (${formal_projects}+)`;
    });
};
count_projects();
